...
if [ "$(uname -s)" != Darwin ]; then
    printf 'Error: this uninstaller supports macOS only.\n' >&2
    exit 1
fi
...
for arg in "$@"; do
    case "$arg" in
        ...
        --yes|-y) assume_yes=1 ;;
        --help|-h)
            printf 'Usage: bash uninstall-macos.sh [--purge-backups] [--yes]\n'
            exit 0
            ;;
        *)
...
if [ "$assume_yes" -ne 1 ]; then
    printf 'This removes the installed Starship/Ghostty configuration and zsh initialization. Continue? [y/N] '
    read -r answer
    case "$answer" in
        y|Y|yes|YES) ;;
        *) printf 'Cancelled.\n'; exit 0 ;;
...
remove_init_line "$zshrc" 'eval "$(starship init zsh)"'

printf 'Removed macOS Evernight configuration.\n'
